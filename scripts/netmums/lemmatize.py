...
def process_data(subforum="all", group="all", id_type="thread_id", per=1, n_chunks=1):
...
def save_data(text, subforum="all", group="all", id_type="thread_id", n_chunks=1):
...
def gen_sql(subforum="all", group="all"):
	sql = '''SELECT p.thread_id AS thread_id, t.id AS id, t.text_clean AS text_clean
		FROM text AS t
		JOIN posts AS p
...
def gen_samp_sql_per(subforum="all"):
	if subforum=="all":
		return ''' SELECT thread_id FROM threads '''
	else:
		return ''' SELECT thread_id FROM threads WHERE subforum="{}" '''.format(subforum)

def gen_sql_per(group="all"):
	sql = '''SELECT p.thread_id AS thread_id, t.message_id AS message_id, t.text_clean AS text_clean
		FROM text AS t
		JOIN posts AS p
		ON t.message_id = p.message_id
